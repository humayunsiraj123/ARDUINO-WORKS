...
unsigned long cardMatchedTime = 0;
unsigned long cardInvalidTime = 0;
unsigned long sensor_update = 0;

const long lockDuration = 3000; // Lock open duration in milliseconds
const long blinkDuration = 1000; // Blink duration for LED feedback in milliseconds
...
if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
  ...
  // Check for lock open duration
  if (cardMatched && (millis() - cardMatchedTime >= lockDuration)) {
    cardMatched = false;
    digitalWrite(lock, HIGH);
    digitalWrite(LED_G, LOW);
  }

  // Check for LED blink duration
  if (cardInvalid && (millis() - cardInvalidTime >= blinkDuration)) {
    cardInvalid = false;
    digitalWrite(LED_R, LOW);
  }
