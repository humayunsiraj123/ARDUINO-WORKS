...
void loop() {
  ...
  { for (int i = 0; i <= 29; i++) //this loop will store whole frame in buffer array.
    {
  ...
  Serial.println(" ");}
    ...
    while(ser_index<30){
      ...
      if(Buffer[ser_index] == 0x5A){
         switch (Buffer[ser_index+4])
      {
        case 0x55:  // soil
          Serial.println("crop type is select");
          Serial.println(Buffer[ser_index+8]);
           tft_crop =Buffer[ser_index+8];
