...
void setup() {
 ...
 // can =CAN0.begin(MCP_ANY, CAN_1000KBPS, MCP_8MHZ) == CAN_OK;
  // put your setup code here, to run once:
while(CAN0.begin(MCP_ANY, CAN_1000KBPS, MCP_8MHZ) != CAN_OK){
  
  ///can = (CAN0.begin(MCP_ANY, CAN_1000KBPS, MCP_8MHZ) == CAN_OK)?1:0;
//  if(can){
  //  Serial.println("CAN_CONNECETED");
   // delay(2000);}
    //else {
      Serial.print("..");
      delay(100);
...
// can=0;
//Serial.println("CAN NOTTTTTTTTTTTT");
//delay(2000);
//
//  }
...
while(!SetBaudRate(0x141,0x01)){
Serial.begin(9600);    //1000mpbs 
delay(100);
}
Brake(0x141,0);//0 for break relase;
delay(1000);}

int SetBaudRate(uint16_t canid, byte baudrate)
{
...
if(cntrl ==1){
  ...
  uint8_t txBuf[8] = {0xA2, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 00};
...
//txBuf[8]=speedControl>>24;
  ...
  int8_t temperature = rxBuf[1];
    ...
    int16_t motorSpeed = (rxBuf[5] << 8) | rxBuf[4];
    int16_t motorAnglex = (rxBuf[7] << 8) | rxBuf[6];
